...
module.exports = Eventer.extend({
    ...
    init: function(queue, rootDir, clientFile, config) {
        ...
        this.baseConfig = config.configs;
    },
    ...
    setConfigByType: function(type, config) {
        this.baseConfig[type] = config;
        _.each(this.workers, function(worker, ID){
            var wType = worker.type == 'cluster' ? 'local' : worker.type;
            if(wType == type){
                worker.execute('setConfig',config,function(){});
            }
        });
        return this.baseConfig;
    },
    ...
    addWorker: function(file) {
        ...
        this.setupWorker(worker, ID, 'local');
    ...
    addServerWorker: function(ws, options) {
        ...
        this.setupWorker(worker, ID, 'server');
        this.workers[ID] = worker;
    ...
    addClientWorker: function(ws, options) {
        ...
        this.setupWorker(worker, ID, 'client');
        this.workers[ID] = worker;
    ...
    setupWorker: function(worker, ID, type) {
        var self = this;
        worker.setConfig(this.baseConfig[type]);
