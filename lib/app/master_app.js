...
module.exports = Eventer.extend({
    ...
    setupServer: function(config) {
        ...
        this.executeNowOrOnce(this.db.ready, 'db.connected', function(){
            ...
            self.websocketServer = new WebsocketServer(self.server, self.workerManager, self.router, self.config.ws);
        });
    },
    ...
    compileJS: function() {
        ...
        var clientjadePath = path.relative(this.rootDir,path.join(__dirname, '..', '..', '..', '.bin', 'clientjade'));
        var cmd = "node " + clientjadePath+" "+templatesPath+" > "+publicJSPath;
